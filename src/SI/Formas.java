	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i < laberinto.length; i++) {
			for (int j = 0; j < laberinto[0].length; j++) {
				...
				System.out.println(java.util.Arrays.toString(c.getVecinos()));
				// Si tiene pared al norte
				if (!c.getVecinos()[0]) {
					x1 = inicio + c.getFila() * grosor;
					y1 = inicio + grosor + c.getColumna() * grosor;
					x2 = x1 + grosor;
					y2 = y1;
				...
				if (!c.getVecinos()[1]) {
					x1 = inicio + grosor + c.getFila() * grosor;
					y1 = inicio + c.getColumna() * grosor;
					x2 = x1;
					y2 = y1 + grosor;
					g.drawLine(x1, y1, x2, y2);
				}
